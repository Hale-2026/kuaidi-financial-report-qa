...
**各步骤预期耗时**（M 系列 Mac，3 并发）：装依赖 ≈ 2–10 分钟（首次，约 120 MB）·
下载 ≈ 3 分钟 · 提取切块 ≈ 8 分钟 ·
...
> `output/一页结论.md`。
>
> 「首次评测」列 = **2026-09-24 检索侧修复前**的实测（当时领域词典未补跨市场术语、
> 尚无分组召回，跨公司全景题 Q01–Q03 三模式全部未命中）。

**⚠ 与台账逐格对齐时别踩这个坑。** `output/eval_table.md` 汇总表里有一列
**「v1 判卷口径@Top5」（当前检索重算）**，数值是 hybrid 70% / bm25 80% /
vector 60% / cover 70%，与上表「首次评测」列（60% / 70% / 60%）**差 10pt**。
**这不是数字错误 —— 两者是不同的量：**

| | 上表「首次评测」列 | 台账「v1 判卷口径」列 |
|---|---|---|
| 检索侧 | 首轮版本（词典未补跨市场术语、无分组召回） | **当前版本** |
| 判卷侧 | 首轮规则（仅正文、不含块元数据） | 同一套首轮规则（仅正文） |
| 性质 | **历史实测**，固化记录，不由脚本重算 | 用当前检索结果**重算**得出 |
| hybrid / bm25 | 60% / 70% | 70% / 80% |

差的正好是 **Q03**（中通 vs 顺丰单票收入）：领域词典补入「票均收入 / 包裹单价」
并重建 BM25 后，该题由未命中转为命中，hybrid 与 bm25 各 +1 题，即 +10pt。
台账第 1 节已把这两个量**并列成两列**（外加一栏逐模式差异来源），可直接对照。
